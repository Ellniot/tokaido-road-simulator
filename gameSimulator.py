...
def OptionsGUI(optionsWin, pNum):
    ...
    pCount = tkinter.IntVar(optionsWin)
    pCountList = {'4','3','5'}
    pCount.set(pNum)
    pTypeList = [tkinter.StringVar(optionsWin) for i in range(pNum)]
    pStratList = [tkinter.StringVar(optionsWin) for i in range(pNum)]

    # add the options GUI elements
    #   make the title and player number selector
    title_lbl = tkinter.Label(optionsWin, text='Tokaido Road').pack()
    #title_lbl.grid(row=0)
    #title_lbl.pack()
    pCount_drpdn = tkinter.OptionMenu(optionsWin, pCount, *pCountList)
    #pCount_drpdn.grid(row=1)
    pCount_drpdn.pack()
    #   make the player options panes based on pNum
    pOptions_pn = tkinter.PanedWindow(optionsWin)
#TODO: make labels 'player 1', 'player 2'...
    player_drpdnList = [tkinter.OptionMenu(optionsWin, pTypeList[i], *PLAYER_LIST) for i in range(pNum)]
    strat_drpdnList = [tkinter.OptionMenu(optionsWin, pStratList[i], *STRAT_LIST) for i in range(pNum)]
    pOptionPair_pnList = [tkinter.PanedWindow(optionsWin, orient='vertical') for i in range(pNum)]
    for i in range(pNum):
    ...
    next_bt = tkinter.Button(optionsWin, text='Next', command=lambda : OpenStratSelector(pCount.get()))
    #next_bt.grid(row=3)
    next_bt.pack()
